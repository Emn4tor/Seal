...
impl ChatNode {
    ...
    /// Shared by every 1:1-Olm-encrypted send (chat messages, group-key
    /// share/request, call signaling): look the recipient up as a contact,
    /// encrypt `payload` under the existing (or newly established, on the
    /// first send) Olm session with them, and hand it to the transport.
    /// Returns the outbound request id so a caller that cares about this
    /// specific send failing (currently just `send_call_invite`/
    /// `send_call_accept`) can correlate it against a later
    /// `OutboundFailure`/`ack: false` — most callers just discard it.
    ...
    /// "Rings" `peer_user_id` for a 1:1 voice call — see `DirectPayload::
    /// CallInvite`'s doc comment. `call_id` is generated by the caller
    /// (`AppService::call_contact`). Tracks the send so a failure to
    /// actually reach them (most commonly: their last-known address is
    /// stale because they're not online right now) is reported as the call
    /// failing, not a generic undelivered message.
    pub fn send_call_invite(&mut self, peer_user_id: &str, call_id: &str) -> anyhow::Result<()> {
        let request_id = self.encrypt_and_send_direct(
            peer_user_id,
            &DirectPayload::CallInvite {
                call_id: call_id.to_string(),
            },
        )?;
        self.pending_call_sends
            .insert(request_id, (call_id.to_string(), peer_user_id.to_string()));
        Ok(())
    }

    /// Same tracking as `send_call_invite`, for the callee's side: our
    /// acceptance failing to reach the caller means the call can't proceed
    /// either.
    ...
    fn handle_request_response_event(
        ...
        match event {
            ...
            request_response::Event::OutboundFailure {
                ...
                if let Some((call_id, call_peer_user_id)) =
                    self.pending_call_sends.remove(&request_id)
                {
                    tracing::warn!(
                        ...
                        call_id,
                        error = %error,
                        "call signaling delivery failed — most likely the peer isn't online"
                    );
    ...
    fn handle_request_response_message(
        ...
        match message {
            ...
            request_response::Message::Response {
                    ...
                    self.pending_call_sends.remove(&request_id)
                {
                    tracing::warn!(
                        ...
                        peer_user_id = %call_peer_user_id,
                        call_id,
                        "peer rejected call signaling (failed to decrypt) — session reset for retry"
                    );
                    return Some(ChatEvent::CallFailed {
                        peer_user_id: call_peer_user_id,
                        call_id,
            ...
            request_response::Message::Response { request_id, .. } => {
                // A successful ack — if this was a tracked call-signaling
                // send, it got through, so there's nothing left to watch
                // for (see `pending_call_sends`'s doc comment); leaving the
                // entry would just leak.
                self.pending_call_sends.remove(&request_id);
                None
            }
        }
